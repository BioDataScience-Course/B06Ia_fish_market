...
Notre jeu de données comporte sept variables (premières et dernières lignes du jeu de données dans le tableau suivant).

<!--% Réalisez un tableau de type "headtail" sur `fish`. -->

```{r tab, warning=FALSE, record='RNP', arg='col_keys'}
tabularise$___
```
...
<!--# Avant d'aller plus loin, avez-vous pris le temps de vous intéresser aux 7 espèces de poissons afin d'avoir une idée de leur morphologie ? -->
...
[] -   La CAH n'est pas très efficace lorsque le nuage de point s'étale de manière continue. Elle a tendance à saucissonner la distribution en sections de longueurs similaires.}-")
```
...
[] -  Aucune des deux CAH n'a réalisé des regroupements représentatifs des différentes espèces.
...
```{r prepakmn, warning=FALSE, record='RODFS', object='fish_scaled'}
sselect(___) %>.%
  ___(.) %>.%
  labelise(., label = list(
...
[] -   Comme pour la CAH, les k-moyennes ont tendance à saucissonner la courbe en segments de longueurs à peu près similaires.}-")
```
...
[] -   Le regroupement k-moyennes est assez similaire à la CAH 2, mais des différences apparaissent toutefois dans quatre des sept groupes k-moyennes (le 2, le 3, le 5 et le 6).
...
[] -   Les regroupements obtenus ne sont pas du tout représentatifs des espèces. 
[] - Les techniques de classification non supervisées  (cah et k-moyennes) ne sont pas les plus adaptées à l'établissement de classifieurs sur base de connaissances *a priori* (ici, l'espèce).
